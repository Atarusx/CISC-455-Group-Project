...
print(f"\nBest weights:")
labels = ["pip_sum", "max_pip", "count_high", "num_hexes", "resource_variety", 
              "pip_variance", "min_pip", "isolation", "has_wood", "has_brick", 
              "has_scarce", "is_edge", "neighbor_opponents"]
for label, w in zip(labels, best_weights):
    print(f"  {label:20s} = {w:.4f}")

print(f"\nBest fitness: {max(stats['best_per_gen']):.4f}")

# Print sigma adaptation summary
print(f"Initial avg σ: {stats['sigma_per_gen'][0]:.4f}")
print(f"Final avg σ:   {stats['sigma_per_gen'][-1]:.4f}")
